...
CREATE TABLE public.cuentas (
    ...
    kilos_totales       NUMERIC GENERATED ALWAYS AS (cantidad_unidades * kg_por_unidad) STORED,
    inversion_total     NUMERIC NOT NULL CHECK (inversion_total > 0),
    precio_venta_kg     NUMERIC NOT NULL CHECK (precio_venta_kg > 0),
...
CREATE TABLE public.ventas (
    ...
    kilos_vendidos  NUMERIC NOT NULL CHECK (kilos_vendidos > 0),
    precio_por_kg   NUMERIC NOT NULL CHECK (precio_por_kg > 0),
    total_venta     NUMERIC GENERATED ALWAYS AS (kilos_vendidos * precio_por_kg) STORED,
...
SELECT
    ...
    COALESCE(SUM(v.kilos_vendidos), 0) AS kilos_vendidos,
    GREATEST(c.kilos_totales - COALESCE(SUM(v.kilos_vendidos), 0), 0) AS kilos_restantes,
